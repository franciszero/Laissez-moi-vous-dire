...
class WritingContentPort(Protocol):
    # lesson=None 表示不限课。写作题和单词一样，本身是独立实体，课只是它的一个属性；
    # 以前 lesson 被当成找文件的路径，于是「不选课就看不到任何作文」。
    def list_tasks(self, lesson: str | None = None) -> tuple[WritingTaskSummary, ...]: ...
    def load_task(self, task_id: str) -> WritingTask: ...
    # 同体裁、跨题共用的素材（scope=task_type/general）。scope 字段一直有人填，
    # 却从来没人读，25 条通用弹药因此锁死在 L33-W1 里。
    def shared_supports(self, task: WritingTask) -> tuple[tuple[str, WritingSupport], ...]: ...
