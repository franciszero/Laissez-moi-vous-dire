...
def test_l39_lesson_is_visible_and_checkpoint_deck_starts(tmp_path):
    """新课写入 vocab/manifest 后，应能在侧栏选课并进入知识点 deck。"""
    db_path = Path("dictation.db")
    backup_path = tmp_path / "dictation.db.bak"
    if db_path.exists():
    ...
    try:
        expected_count = len(manifest.checkpoints(manifest.load("../L39/manifest.json")))

        at = AppTest.from_file("app.py", default_timeout=10)
        at.run()
        assert not at.exception

        at.selectbox(key="sel_lesson").set_value("L39").run()
        assert not at.exception
        start = next((b for b in at.button if b.label.startswith("开始这一课（")), None)
        assert start is not None
        n = int(start.label.split("（")[1].split(" 词")[0])
        ...
        knowledge_button.click().run()
        assert not at.exception
        assert at.session_state.cp_label == "知识点 · L39"
        assert len(at.session_state.cp_cards) == expected_count
        assert f"📝 知识点 1/{expected_count}" in [s.value for s in at.subheader]
    finally:
        if backup_path.exists():
...
def test_l38_lesson_is_visible_and_checkpoint_deck_starts(tmp_path):
    """新课写入 vocab/manifest 后，应能在侧栏选课并进入知识点 deck。"""
    db_path = Path("dictation.db")
    backup_path = tmp_path / "dictation.db.bak"
    if db_path.exists():
    ...
    try:
        expected_count = len(manifest.checkpoints(manifest.load("../L38/manifest.json")))

        at = AppTest.from_file("app.py", default_timeout=10)
        at.run()
        assert not at.exception

        at.selectbox(key="sel_lesson").set_value("L38").run()
        assert not at.exception
        start = next((b for b in at.button if b.label.startswith("开始这一课（")), None)
        assert start is not None
        n = int(start.label.split("（")[1].split(" 词")[0])
        ...
        knowledge_button.click().run()
        assert not at.exception
        assert at.session_state.cp_label == "知识点 · L38"
        assert len(at.session_state.cp_cards) == expected_count
        assert f"📝 知识点 1/{expected_count}" in [s.value for s in at.subheader]
    finally:
        if backup_path.exists():
