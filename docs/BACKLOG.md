...
## 10. 测试收尾不许用「删这个词最近一条记录」这类语义

**状态**：已修，但教训要留着。

2026-08-21，`tests/test_scan_ui.py` 里几条用例在 `finally` 里循环调用
`store.delete_last_scan_attempt(word_id, skill)` 收尾。这个函数删的是**该词最近一条
扫读记录**，它分不清那条是测试刚造的、还是用户真背出来的。

用户扫读记录为 0 时它无害；用户一开始用速过，它就变成破坏性的。**实际后果**：
全量跑了一次之后，用户 13 条真实扫读记录变成 12 条，丢的是 `attempts.id = 3103`
（时间戳在 `essayer` 和 `faible` 之间）。**无法恢复**——不知道是哪个词、判的是会
还是不会；当时手上的备份早于那次学习，拿它还原会毁掉更多。

同一批用例还有第二个毛病：断言写成「库里这两个词的扫读记录**等于**这两条」。
用户一旦背过这两个词，断言必然失败——测试会随着用户使用而越来越飘。

**已改成**：一个 `scratch` fixture 记下 `MAX(attempts.id)` 水位线，断言只看水位线
以上的行，收尾只 `DELETE ... WHERE id > 水位线 AND skill LIKE 'rec\_%'`。跑前跑后
用户记录数不变，已实测。

**规矩**：任何在真实 `dictation.db` 上跑的用例，收尾只许按「本次造出来的 id」删，
不许按「某个词的最近一条」删；断言只许看增量，不许看绝对集合。更彻底的做法是让
UI 用例跑在临时库上（见第 9 条），那条还没做。
