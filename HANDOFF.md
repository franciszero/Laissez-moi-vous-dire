...
| 文件 | 作用 | 何时去读 |
|---|---|---|
| `app.py` (~2050 行) | 唯一 Streamlit 入口：UI、状态机、各模式渲染、DB 读写 | 改 UI/模式/流程 |
...
| `llm.py` | 薄 rapid-mlx 适配器：读 Hermes 模型、按需启停服务、`chat(prompt)` 通用调用；模型/thinking 可用 env(`DICTATION_LLM_MODEL`/`_THINKING`)覆盖（会话级，非 per-call）| 改本地 LLM 对接/生命周期 |
| `aigrade.py` | AI 精练「确定性」部分（无网络、可单测）：`build_prompt`(按固化规范)、`normalize_result`、`anchor_and_segment`(把模型片段在原句逐字验证再染色)、`to_html` | 改批改 prompt/染色逻辑 |
| `conjugate.py` | 确定性法语【规则】动词变位(er/ir2/re，由显式 type 驱动、不靠词尾猜)：动词变位卡的**答案键来源，绝不用 LLM**。不规则动词不在此生成 | 改变位规则/加时态/加动词类型 |
| `anki.py` | AnkiConnect **只读**取词卡；`_find_note` 无把握返回 None；`card_state`→ok/stub/missing（stub=生成失败的全 N/A 残卡，隐藏+提示）；`memoized_state` 只缓存 ok（新生成的卡免重启即现）| 改 Anki 集成 |
| `docs/MEMO-2026-07-08-anki-integration.md` | **Anki 读侧模型 + 生成侧(anki-wordsmith)词条格式 + 调查手册/证据命令**；两个读侧 bug 复盘 | 补卡/查 Anki 集成/生成词条 |
...
| `docs/MEMO-2026-06-25-l23-post-pipeline.md` | L23 转录 pipeline 之后的知识点盘点→8501 落卡→网站可见性复盘 | 做 L24+ 同类课后上线或查“pipeline 后还要干嘛” |
| `scripts/backfill_fem.py` | 给现有 `../L*/vocab.json` 回填 `fem`/`fem_raw`（一次性，已跑过）| 加新课/改重建规则后 |
| `scripts/merge_vocab.py` | JSON 批次按 lemma 去重追加到某课 vocab；已有整条不覆盖，冲突批次报错，幂等 | 从知识点卡/外部清单归位词汇 |
...
| `scripts/llm_grade_bakeoff.py` | 离线金标准 bakeoff：跑 AI 精练 pipeline 按【类型】打分（错句漏标=最高危/对的误判/JSON率/延迟），种子 `scripts/gold/l22_ai_gold.json` | 选 LLM 模型/是否开 thinking |
| `tests/` | pytest，**94 个，全绿**，含 Streamlit AppTest | 每次改完跑 |
| `../L*/conjugation.json` | 每课"老师讲过的规则动词"清单(verb/type/zh)→动词变位卡。新课照此加 | 加动词变位卡 |
...
- ✅ **新课/改文件自动可见 + 重新扫描兜底**：`load_vocab` / `load_checkpoints` / `load_conjugations` 都用 lesson data file signature（相对路径 + mtime + size）作为 `@st.cache_data` key；pipeline 或别的 session 写入 `../L*/vocab.json` / `manifest.json` / `conjugation.json` 后，下一次 rerun 会自动重新读。vocab 导入 SQLite 也按同一 signature 补跑，确保新词不只在侧栏可见，也真的能被「开始这一课」抽到。侧栏「🔄 重新扫描词表」仍保留，作为强制 `clear()` 兜底。
- ✅ **知识点 checkpoint 复习 + lesson manifest**：每课一份 `../L*/manifest.json`（机器合同，每 chunk 强制归桶 vocab|drill|checkpoint|skip → 覆盖率可清点）。`manifest.py`(validate/load/checkpoints/vocab_items)、`srs.py`(next_schedule，词与卡共用的间隔 [1,2,4,7,15,30])、`scripts/coverage_report.py`、`scripts/build_checkpoints_from_species.py`。8501 侧栏「📝 知识点(N)」→ `render_checkpoint`：有 answer 机判(check_fr)、无则揭示背面自评；SRS 状态存 DB `checkpoints` 表(card_id 主键)，复用 `srs.next_schedule`。**checkpoint 是独立流程**：`cp_active/cp_cards/cp_index/cp_show_back/cp_feedback`，**绝不碰** `pool/current_word/render_practice`（主区 `if cp_active: render_checkpoint() else: …`）。L21 已升级为正式复习 deck：94 vocab + **85 张知识点卡**，其中 **73 张 reviewed species checkpoint 卡**（`coverage.expected_species_count=73`，`coverage_report.py` 显示 `species 覆盖: 73/73`）+ **12 张混合代词作业复习卡**（`mixed-pronoun-review`，不含原 30 题逐题答案，不计入 species 分母）。**卡片精修**走 `../L21/L21.card_overrides.json`（按 `species_label` 覆盖 front/back/answer），`build_checkpoints_from_species.py --overrides` 合并；卡 `id` 稳定 → 重建不丢精修也不丢 SRS 进度。**73 张 species 卡已全部精修：49 机判（answer→check_fr 填空）+ 24 中文自评，0 通用模板。** 新课照此：`build_checkpoints_from_species.py --overrides <课>.card_overrides.json`。`drill` 桶留位未实现；`record_attempt` 暂未改用 srs（DRY 重构故意推迟，行为不变）。设计/计划见 `docs/specs|plans/2026-06-17-*`、`docs/MEMO-2026-06-17-*`。
- ✅ **L22「🤖 AI 精练」（拆解式批改）**：**【2026-06-24：已并入「📝知识点」deck 作为 `kind=production` 卡（`render_checkpoint`→`_render_production_card`）；独立 `llm_active` 流程/🤖按钮已删。模型改为**提交时**加载，`_llm_idle_watch` 提到顶层兜底卸载。详见 `docs/plans/2026-06-24-handoff.md`】** **绝不写 Hermes 配置**。下述判分设计仍是定稿（别推翻）：
  - **1-4 是出题时固化的「标准规范」**（`LLM_EXERCISES` 每项带 `target/candidates(开放候选)/conjugations(只本句)/structures/references/required/ignore` + `status:draft`），**运行时不让本地模型现编参考**（自生成参照=相关性错误）。`status≠reviewed` 的规范 UI 标「AI 起草未核验」，待人工改 `reviewed`。
  - **5-8 是运行时本地模型按规范批改**：`aigrade.build_prompt`→`llm.chat`(**一次调用**)→`aigrade.normalize_result`。状态是**开放集**(正确/可接受变体/正确但不自然/错误/多余/拿不准)——规范是参照不是白名单。
  - **第 8 步染色 = 代码确定性**：模型给「片段+状态」，`aigrade.anchor_and_segment` 用子串在原句**逐字定位验证、忽略模型偏移、丢弃幻觉片段**，再 `to_html` 四色(绿接受/黄不自然/红错/灰删除线多余)，标「AI 批改·需核验」。`difflib` 不冒充对错。
  - **三态终判**：我对/我错/**拿不准**(不写 SRS)；**用了提示的「我对」不算独立通过**(assisted → 不给 SRS 学分)。提示等级(词表/变位/句式)记进历史。
  - **历史回翻**：`store.ai_attempts` 表(自包含，存 spans 可重放染色)，同题「📖 上次→这次」对比。
  - **选模型靠离线 bakeoff**：`scripts/llm_grade_bakeoff.py` + `scripts/gold/`，**当前默认仍 35B-A3B**；27B 更准但 ~5.4× 慢。真实 35B：加载 10–12s、批改 2.6–3.2s、+~1.4GB swap、卸载后内存 47→25GB，**故严禁长驻**。⚠️ **本地模型批改的术语解释会自信地出错**——它是陪练不是教科书，权威变位/语法靠确定性机判卡 + 老师。
- ✅ **动词变位卡**：**【2026-06-24：已并入「📝知识点」deck 作为 `kind=conj` 卡（`render_checkpoint`→`_render_conj_card`），独立 `conj_active` 流程已删】**。**一动词一张卡**、整张范式（présent/imparfait/futur proche/futur simple/conditionnel/过去分词）8 格（je/tu/il/elle/nous/vous/ils/elles，elle≈il、elles≈ils）手敲填空、提交后逐格 `check_fr` 核对、可"再敲一遍"。**答案键由 `conjugate.py` 确定性生成（规则 er/ir2/re + 人工核对不规则表 être/avoir/aller/voir/pouvoir/vouloir），绝不用 LLM**（mlconjug3 在本机坏了：其模型要 sklearn 1.6.x、本机 1.9.0；详见 `docs/plans/2026-06-24-handoff.md`）。动词清单 `../L*/conjugation.json`（`status:draft` 待核验）。复用 checkpoints SRS + `checkpoint_attempts` 历史。形容词性数配合卡为后续同法任务。
...
| 3 | ✅ L21 species deck 精修（完成）| — | 73/73 **全部精修**：49 机判 + 24 中文自评，0 通用模板（`数据仓库/L21/L21.card_overrides.json`）。后续：新课同法精修；可把自评卡进一步拆 drill |
| 4 | ✅ L21 混合代词作业复习卡（完成） | — | 12 张 `mixed-pronoun-review` 自评卡，排在 L21 知识点开头；不泄露原 30 题逐题答案 |
| 5 | 用户反馈后微调 | agent | 等用户用过：打字模式 / 左侧边栏视觉 / 形义音热力图 / 防刷分 / 自动模式 / 知识点卡 |
...
- **掌握度**：`mastery_score` = 按天取当天第一次 → 半衰期(14天)加权 → Wilson 下界(Z=1.28)。`overall`=min(form,meaning,pron)。颜色：0灰`#e6e6e6`→中黄`#ffd54f`→1绿`#66bb6a`。「同日先错后对色块不动」＝设计（当天第一次为准防刷分），非 bug；`✅`(本轮)与色块(耐久掌握)是两个信号。用户直觉「应平滑」的**平滑证据权重版**（间隔因子×半衰期）已设计验算但**暂不实现**：`docs/specs/2026-07-08-smooth-mastery-design.md`。
...
- **Anki 只读**：`anki.py` 经 AnkiConnect(localhost:8765) 读「Français」牌组；匹配不确定时**返回 None**（曾因 fallback `infos[0]` 返回错词卡，已删）。`card_state`＝ok/stub/missing（stub=生成失败的全 N/A 残卡→隐藏+「待重生成」提示）；`memoized_state` **只缓存 ok**（负结果永久缓存过 bug，`3bbfb62`）。**卡片内容不在本仓生成**——由外部 `anki-wordsmith` 制卡；读侧模型/生成侧词条格式/调查手册全在 `docs/MEMO-2026-07-08-anki-integration.md`。
- **念法语状态机**（`render_speak`，app.py ~838）：idle→armed→(judge)→done。armed 时读 `asr.read_latest()` 且要求 `ts > armed_at`（防读到上一句的旧转写）。
- **课程拥有自己的知识卡；跨课知识用「重复 + 轻指针」，不建共享基建。** 同一知识点跨课讲（如时态在 L20/L22），在**需要系统学它的那一课**里建一套卡（那课的资产），别课保留自己的卡当历史记录。**不要**做 global card ID / 跨课 registry / 共享掌握度模型——单用户 app，重复几张卡比维护共享层便宜。只有当 3+ 课重教同一块知识、且掌握度被拆/多 deck 反复复习造成**实际痛感**时，才考虑升级共享模型。（通用版见 `existing-pattern-first` 的 "Premature cross-cutting abstraction"。）
- **撇号只在判分时归一，绝不改存储**：`lemma` 是 DB(`words.text`)、`vocab.json`、外部 Anki 笔记三处共用的关联键；弯/直撇号统一只在 `matcher.norm_fr` 里做（`_APOS`），改任何一处存储都会让三者失联+重复导词（踩过，见 §6）。
...
6. **新课可见性现在靠文件签名自动失效，别再假设必须手动清 cache**：`load_vocab` / `load_checkpoints` / `load_conjugations` 的 cache key 包含 lesson data 文件签名；`import_vocab_into_db` 也按 vocab signature 补跑。加课或改 manifest 后，正常 rerun 应自动看到并导入新词；若用户浏览器仍显示旧状态，先刷新页面，再点侧栏「🔄 重新扫描词表」或重启。L23 曾因旧无参数 cache 看不到，后又暴露“文件可见但 DB 未补导入”的风险，已由 `tests/test_checkpoint_ui.py::test_l23_lesson_is_visible_and_checkpoint_deck_starts` 覆盖（断言 L23 知识点可进入，且「开始这一课」词数等于 manifest vocab 数）。
