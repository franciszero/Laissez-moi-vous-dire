...
- ✅ **新课/改文件自动可见 + 重新扫描兜底**：`load_vocab` / `load_checkpoints` / `load_conjugations` 都用 lesson data file signature（相对路径 + mtime + size）作为 `@st.cache_data` key；pipeline 或别的 session 写入 `../L*/vocab.json` / `manifest.json` / `conjugation.json` 后，下一次 rerun 会自动重新读。侧栏「🔄 重新扫描词表」仍保留，作为强制 `clear()` 兜底。
...
6. **新课可见性现在靠文件签名自动失效，别再假设必须手动清 cache**：`load_vocab` / `load_checkpoints` / `load_conjugations` 的 cache key 包含 lesson data 文件签名。加课或改 manifest 后，正常 rerun 应自动看到；若用户浏览器仍显示旧状态，先刷新页面，再点侧栏「🔄 重新扫描词表」或重启。L23 曾因旧无参数 cache 看不到，已由 `tests/test_checkpoint_ui.py::test_l23_lesson_is_visible_and_checkpoint_deck_starts` 覆盖。
