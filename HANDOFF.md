...
| `llm.py` | 薄 rapid-mlx 适配器：读 Hermes 当前模型、按需启停服务、自由产出 JSON 批改 | 改本地 LLM 对接/生命周期 |
...
| `tests/` | pytest，**73 个，全绿**，含 Streamlit AppTest | 每次改完跑 |
...
- ✅ **L22「🤖 AI 精练」**：独立 `llm_active` 流程，绝不碰 `pool/current_word`。进入时由 `llm.py` 读取 `~/.hermes/config.yaml` 的当前模型并启动 rapid-mlx；退出、切换到非 AI 入口、闲置 5 分钟或 app 退出时卸载。5 项自由产出覆盖“简单正确主干→同 cue 完整升级”与 3 个长句翻译；模型用 `response_format=json_object` 给建议，只有用户「我对/我错」才写现有 checkpoint SRS。真实 35B 测量：加载约 10–12 秒、批改约 2.6–3.2 秒；与 8501 共存会新增约 1.4 GB swap，退出后 15 秒物理内存从 47 GB used 回落到 25 GB used，故严禁改成长驻。
