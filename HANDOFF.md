...
- ✅ **L22「🤖 AI 精练」（拆解式批改，重构版）**：独立 `llm_active` 流程，绝不碰 `pool/current_word`；生命周期(进入加载/退出/切换/闲置5分钟/atexit 卸载)见 `llm.py`，**绝不写 Hermes 配置**。设计（决策定稿，别推翻）：
...
- ✅ **动词变位卡（🔠 动词变位）**：独立 `conj_active` 流程（互斥靠侧栏分面渲染，同 cp）。**一动词一张卡**、整张范式（présent/imparfait/futur proche/futur simple/conditionnel/过去分词）8 格（je/tu/il/elle/nous/vous/ils/elles，elle≈il、elles≈ils）手敲填空、提交后逐格 `check_fr` 核对、可"再敲一遍"。**答案键由 `conjugate.py` 确定性生成（只规则动词 er/ir2/re），绝不用 LLM**（无可用变位库；不规则动词需核对过的表，暂未收）。动词清单 `../L*/conjugation.json`（=老师讲过的规则动词，`status:draft` 待核验）。复用 checkpoints SRS + `checkpoint_attempts` 历史。形容词性数配合卡为后续同法任务（先只做动词）。
