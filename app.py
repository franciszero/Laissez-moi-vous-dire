...
if "conj_active" not in st.session_state:  # 「🔠 动词变位」独立流程，互斥靠侧栏分面渲染（同 cp）
    st.session_state.conj_active = False
    st.session_state.conj_cards = []
    st.session_state.conj_index = 0

if "llm_active" not in st.session_state:  # 「🤖 AI 精练」同样独立，不碰 pool/current_word
...
with st.sidebar:
    ...
    if st.button(f"🔁 知识点·到期（{len(_due_cp)}）", disabled=not _due_cp,
        ...
        st.session_state.conj_active = False
        st.session_state.cp_active = True
        st.session_state.cp_cards = _due_cp
        st.session_state.cp_index = 0
        st.session_state.cp_show_back = False
...
def _render_llm_exercise() -> None:
    ...
    _render_lookback(exercise["id"])
    hint_level = _render_hints(exercise)        # 始终渲染：条件渲染带 key 的控件会跨 rerun 丢状态
    if has_result:
        _render_spec_review(exercise)           # 参考句只在提交后揭示
    ...
    if submitted:
        ...
        if not answer.strip():
            ...
            with st.spinner("批改中…"):
                try:
                    raw = llm.chat(aigrade.build_prompt(exercise, answer.strip()))
    ...
    final_ok = c_ok.button("✅ 我对", type="primary", key="llm_self_right")
    final_wrong = c_wrong.button("❌ 我错", key="llm_self_wrong")
    final_unsure = c_unsure.button("🤔 拿不准", key="llm_self_unsure")
    if final_ok or final_wrong or final_unsure:
        verdict = "我对" if final_ok else "我错" if final_wrong else "拿不准"
        # 我错照常记错；用了提示的「我对」不算独立通过 → 不给 SRS 学分；拿不准不写 SRS
        if final_wrong:
            update_checkpoint(exercise["id"], False)
        elif final_ok and not assisted:
            update_checkpoint(exercise["id"], True)
        save_ai_attempt(
            exercise["id"], st.session_state.get("llm_graded_answer", ""),
            verdict=verdict, minimal=result.get("最小修正", ""),
            natural=result.get("更自然版", ""), feedback=result.get("spans", []),
            hint_level=st.session_state.get("llm_hint_level", "无"),
...
def render_llm_practice() -> None:
    ...
    if st.session_state.get("llm_loaded") and not llm.is_loaded():
        st.session_state.llm_loaded = False
        st.session_state.llm_error = "本地模型服务意外停止。请重试加载。"

    if not st.session_state.get("llm_loaded") and not st.session_state.get("llm_load_attempted"):
        st.session_state.llm_load_attempted = True
        with st.spinner("加载本地模型中…首次加载可能需要一会儿"):
            try:
                elapsed = llm.load()
                st.session_state.llm_loaded = True
                st.session_state.llm_last_active = time.time()
                st.session_state.llm_load_seconds = elapsed
                st.session_state.pop("llm_error", None)
            except llm.LLMError as exc:
